 ...
 * 하단 고정 신청 바.
 * - 히어로를 지나 스크롤하면 나타남
 * - 신청 폼(#apply)이 화면에 보이면 숨김 (버튼 중복 방지)
 */
export default function StickyCTA() {
  const [show, setShow] = useState(false)
  const [nearApply, setNearApply] = useState(false)
  ...
  useEffect(() => {
    ...
    if (apply) {
      obs = new IntersectionObserver(
        ([entry]) => setNearApply(entry.isIntersecting),
        { threshold: 0.05 }
      )
      obs.observe(apply)
    }
