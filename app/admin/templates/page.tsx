  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-2">
        <h1 className="text-xl font-bold text-[#3b2e21]">📋 DM 리포트 템플릿</h1>
      ...
      <p className="text-sm text-gray-500 mb-8 leading-relaxed">
        ...
        <strong className="text-[#3b2e21]"> {'{이름}'}만 실제 이름으로 바꾼 뒤</strong> DM
        또는 문자로 보내주세요. 24시간 안에 보내는 게 신뢰의 핵심이에요.
      </p>

      <div className="space-y-6">
        {REPORT_TEMPLATES.map((t) => (
          <div key={t.key} className="bg-[#fdfaf4] rounded-2xl border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold text-[#3b2e21]">
                {t.emoji} {t.title}
              </h2>
              <button
            ...
            <pre className="whitespace-pre-wrap text-sm leading-relaxed text-gray-600 bg-[#f3ece1] rounded-xl p-5 font-[inherit]">
              {t.body}
            </pre>
          </div>
        ))}
